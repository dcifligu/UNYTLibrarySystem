class BookDecorator < ApplicationDecorator
  ...
  def decorated_author
    "<p class='mt-2 text-gray-500'>#{author}</p>".html_safe
  end
  ...
  def decorated_details
    content = ""
    content += "<p class='text-sm text-gray-500'>Publisher: #{publisher}</p>" if publisher.present?
      ...
      <div class="max-w-6xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
          <div class="p-8">
            ...
            #{decorated_details}
            <div class="mt-6">
              #{view_context.link_to "Reserve", view_context.reserve_book_path(object), class: "inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"}
            </div>
          </div>
        </div>
      </div>
